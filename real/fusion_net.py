...
class SSF(nn.Module):
    def __init__(self, hsi_c, pan_c,up_scale):
        ...
        self.half_k = nn.Parameter(torch.tensor(3.5),requires_grad=True)
    ...
    def forward(self):
        ...
        all_spak = create_gaussian_kernel(sigma, k_size=(25, 25))
...
class AWSCA(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.pan_conv = nn.Conv2d(1, channel, 1, bias=False)
        self.lr_conv = nn.Conv2d(16, channel, 1, bias=False)
        ...
        self.conv_down = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(channel, channel, (5, 5), (2, 2), 2)
        )
...
class Fusion(nn.Module):
    def __init__(self, hsi_chnnel=16, pan_channel=1, num_res=6, num_fm=64,up_scale=2):
        ...
        self.lr_ps = nn.PixelShuffle(4)

        self.ps=nn.PixelShuffle(2)
        self.psconv=nn.Conv2d(32, 64, kernel_size=1, stride=1)


        self.cat1=nn.Sequential(
            nn.Conv2d(5, 32, kernel_size=1, stride=1),
        )
        self.ligth_conv1=Light_Conv(self.msfa_size,32)


        self.cat2 = nn.Sequential(
            nn.Conv2d(17, 32, kernel_size=1, stride=1),
        )

        self.ligth_conv2=Light_Conv(self.msfa_size,32)

        self.attconv1=Att_Conv(32,4)
        self.attconv2 = Att_Conv(32,4)
        # self.attconv3 = Att_Conv(32,4)
        # self.attconv4 = Att_Conv(32,4)

        self.final_conv = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1)

    def forward(self,lr,pan):
        lr_up=torch.nn.functional.interpolate(lr,scale_factor=self.up_scale,mode='bicubic')

        #w
        pan_f1=self.pan_ups(pan)     # 64*64*1-->32*32*4

        lr_f1=self.lr_ps(lr)     # 8*8*16-->32*32*1
...
class D_F_net(nn.Module):
    def __init__(self, hsi_spectral=16, pan_chenenl=1,num_res=6, num_fm=64,up_scale=2):
        ...
        self.ssf_net=SSF(hsi_spectral, pan_chenenl,up_scale)
        self.up_scale=up_scale
    ...
    def spa_and_spe_d(self, lrhsi,pan):
        ...
        pan_d1 = torch.matmul(lrhsi.permute(0, 2, 3, 1), spek1).permute(0, 3, 1, 2)  #

        #
        pan_d2 = F.conv2d(pan, spak1[0].unsqueeze(0), bias=None, stride=(1, 1), padding=spak1.shape[2] // 2, dilation=1,
                           groups=1)  #
        pan_d2 = pan_d2[:, :, 0:: self.up_scale, 0:: self.up_scale]  #
    ...
    def fusion_d(self, fusion_hsi):
        ...
        spak1, spek1, half_k, sigma = self.ssf_net()
        pan_p1 = torch.matmul(fusion_hsi.permute(0, 2, 3, 1), spek1).permute(0, 3, 1, 2)  #

        #
        lrhsi_p2 = F.conv2d(fusion_hsi, spak1, bias=None, stride=(1, 1), padding=spak1.shape[2] // 2, dilation=1,
                           groups=16)  #
        lrhsi_p2 = lrhsi_p2[:, :, 0:: self.up_scale, 0:: self.up_scale]  #
...
if __name__ == '__main__':
    ...
    hsi = torch.randn((1, 16,255, 255), device=device)
    msi = torch.randn((1,1, 2040, 2040), device=device)

    model2 = D_F_net(16,1,6,64,8).to(device)
    model2.train()
    ...
    for i in range(50):
        ...
        with torch.no_grad():
            time1 = time.time()
            fusion_hsi= model2.fusion(hsi, msi)
            time2 = time.time()
