...
if __name__ == '__main__':
    ...
    scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer1, [30,60], 0.5)
    ...
    for epoch in range(start_epoch + 1, EPOCH + 1):
        ...
        for  train_lrhs,train_hrpan in loop:
            ...
            fusion_hsi,pan_d1, pan_d2, pan_p1, lrhsi_p2, half_k, sigma = cnn.fusion(train_lrhs, train_hrpan)

            # 损失1：低分辨率高光谱损失
            lr_loss = 1-ssim_func(lrhsi_p2, train_lrhs)
            # 损失2：全色损失
            pan_loss =loss_func(pan_p1, train_hrpan)

            # 再次采样的损失
            down_loss=1-ssim_func(pan_d1, pan_d2)

            pand_g = mygradient(pan_d1, 1, 'cuda')
            pan_g = mygradient(pan_d2, 1, 'cuda')
            ...
            # TVLOSS
            tv_loss=0.1*total_variation_loss(fusion_hsi)
            ...
            loss_all1 = 0.1*lr_loss + pan_loss+tv_loss+g_ssim_loss1+g_ssim_loss2
            ...
            all_losses1.update(loss_all1.detach().cpu().numpy())
            all_losses2.update(loss_all2.detach().cpu().numpy())

            lr_losses.update(lr_loss.detach().cpu().numpy())
            pan_losses.update(pan_loss.detach().cpu().numpy())
            down_losses.update(down_loss.detach().cpu().numpy())
            tv_losses.update(tv_loss.detach().cpu().numpy())
            g_ssim_losses1.update(g_ssim_loss1.detach().cpu().numpy())
            ...
            loop.set_postfix({
                ...
                'pan_loss': f'{pan_losses.avg.item():.6f}',
                'tv_loss': f'{tv_losses.avg.item():.6f}',
                'lr_loss': f'{lr_losses.avg.item():.6f}',
                'g_ssim_loss1': f'{g_ssim_losses1.avg.item():.6f}',
                'g_ssim_loss2': f'{g_ssim_losses2.avg.item():.6f}',
