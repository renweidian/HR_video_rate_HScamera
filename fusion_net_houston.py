...
class AWSCA(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.pan_conv = nn.Conv2d(4, channel, 1, bias=False)
        self.lr_conv = nn.Conv2d(144, channel, 1, bias=False)
...
class Fusion(nn.Module):
    def __init__(self, hsi_chnnel=16, pan_channel=1, num_res=6, num_fm=64,up_scale=2):
        ...
        self.final_conv = nn.Conv2d(hsi_chnnel*2, 144, kernel_size=3, stride=1, padding=1)
...
class D_F_net(nn.Module):
    def __init__(self, hsi_spectral=16, pan_chenenl=1,num_res=6, num_fm=64,up_scale=2):
        ...
        self.ssf_net=SSF(144, 4,up_scale)
        self.up_scale=up_scale
    ...
    def spa_and_spe_d(self, lrhsi,pan):
        ...
        pan_d2 = F.conv2d(pan, spak1[0:4,:,:,:], bias=None, stride=(1, 1), padding='same', dilation=1,
                           groups=4)  # 空间模糊
...
if __name__ == '__main__':
    ...
    hsi = torch.randn((2, 144,16, 16), device=device)
    msi = torch.randn((2,4, 64, 64), device=device)

    model2 = D_F_net(144,4,6,64,4).to(device)
    model2.train()
